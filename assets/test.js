...
function activateTab(tabId) {
    ...
    const btnUploadList = document.getElementById('btn-upload-list');
    ...
    [btnPullNid, btnDataEntry, btnGenerateNotice, btnGenerateList, btnUploadList, btnStartNew, btnImport, btnInjectRtgs, btnInjectEftn, btnScanRtgs, btnRestoreCbs, btnSmartMode, btnClearForm].forEach(b => {
        if (b) {
    ...
    if (isNidPortal) {
        ...
        if (isCalculator) {
            ...
            if (btnGenerateList) {
                btnGenerateList.classList.remove('hidden-panel');
                btnGenerateList.style.display = 'inline-block';
            }
            if (btnUploadList) {
                btnUploadList.classList.remove('hidden-panel');
                btnUploadList.style.display = 'inline-block';
            }
            if (formActionsSection) formActionsSection.classList.remove('hidden-panel');
        } else {
    ...
    if (openTabs[tabId]) {
        ...
        openTabs[tabId].contentElement.style.display = 'flex'; // Use flex for centering content
        activeTabId = tabId;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.addCustomerFromPulledNidData = function() {
        ...
        if (!nid && !nameBn && !nameEn) {
            alert('অনুগ্রহ করে কমপক্ষে নাম অথবা এনআইডি নম্বর প্রদান করুন।\n(Please provide at least a name or NID number.)');
            return;
        }
    ...
    setupNavLink('btn-open-user-info', () => {
        document.getElementById('userInfoModal').classList.add('visible');
    }, 'Open User Info');

    setupNavLink('btn-open-branch-info', () => {
        document.getElementById('branchInfoModal').classList.add('visible');
    }, 'Open Branch Info');
    ...
    window.getFormLogic = function(win) {
        if (!win) return null;
        try {
            return win.SBACFormLogic || win.AgriLoanLogic || win.CmsmeLoanLogic || win.cmsmeApp || win.DPSMSSFormLogic || win.RTGSFormLogic || win.EFTNFormLogic || win.InterestCalcLogic || win.DepositLoanLogic;
        } catch (e) {
            return null;
        }
    };
    ...
    document.getElementById('btn-generate-list')?.addEventListener('click', () => {
        const win = getActiveIframe()?.contentWindow;
        if (typeof win?.generateBorrowerList === 'function') {
            win.generateBorrowerList();
        } else if (typeof win?.generateList === 'function') {
            win.generateList();
        } else {
            console.warn('List generation function not found on iframe window');
        }
    });

    document.getElementById('btn-upload-list')?.addEventListener('click', () => {
        const win = getActiveIframe()?.contentWindow;
        if (typeof win?.triggerFileUpload === 'function') {
            win.triggerFileUpload();
        } else if (typeof win?.uploadList === 'function') {
            win.uploadList();
        } else {
            console.warn('Upload function not found on iframe window');
        }
    });
    ...
    document.getElementById('btn-save-work')?.addEventListener('click', () => {
        ...
        window.ipcRenderer.invoke('app-full-export', allLocalData, suggestedName).then(res => {
            if (res.success) alert(`Backup successfully created at: ${res.path}
...
You can now take this file to another branch.`);
    ...
    document.getElementById('btn-load-work')?.addEventListener('click', () => {
        ...
        window.ipcRenderer.invoke('app-full-import').then(res => {
            if (res.success) {
                ...
                alert('System restored successfully. The application will now reload to apply settings.');
    ...
    document.getElementById('btn-reset-suite')?.addEventListener('click', () => {
        ...
        window.ipcRenderer.invoke('app-reset-data').then(res => {
            if (res.success) {
                ...
                alert('System reset successfully. The application will now exit.');
                window.close();
            } else {
    ...
    function populateActiveForm(customer, targetContext) {
        if (!customer) {
            alert('No customer selected to populate.');
            return;
        }
        ...
        if (event.data.command === 'EXECUTE_SHELL_ACTION') {
            switch (event.data.actionId) {
                case 'open-customer-modal':
                    ...
                    break;
                case 'edit-customer':
                    if (!event.data.nid) { alert('গ্রাহকের NID পাওয়া যায়নি।'); break; }
                    openLocalForm(
                ...
                case 'populate-customer':
                    if (!event.data.nid) {
                        alert('Customer identifier missing.');
                        return;
                    }
            ...
            window.DB.saveCustomer(event.data.customer).then((res) => { if (res && res.success === false) { window.showAppToast('Error saving customer: ' + res.error, true); return; }
...
function startSmartNoticeTracker() {
    const checkNotices = () => {
        ...
        // This is a placeholder logic. When the user finishes designing the Borrower List,
        // this will iterate over all active loans and check their expiry date against today.
        ...
        if (!borrowers || borrowers.length === 0) {
            // No borrowers found, don't show any dummy warnings
            scrollArea.innerHTML = '';
            return;
        }

        // Dummy alert to show the UI is wired and working in the Task/Alert container.
        const pendingNoticesHtml = `
            <div class="task-item pending" style="border-left: 4px solid #f39c12; padding: 10px; margin-bottom: 8px; background: #fff; cursor: pointer;" onclick="document.getElementById('nav-monitoring-demand').click()">
                <strong>Notice Pending:</strong> A/C-0123456789 (Demand Notice) - 3 months to expiry.
            </div>
            <div class="task-item pending" style="border-left: 4px solid #e74c3c; padding: 10px; margin-bottom: 8px; background: #fff; cursor: pointer;" onclick="document.getElementById('nav-monitoring-final').click()">
                <strong>Notice Pending:</strong> A/C-9876543210 (Final Notice) - Exceeded expiry.
            </div>
        `;
        
        scrollArea.innerHTML = pendingNoticesHtml;
    };
    
    checkNotices();
    setInterval(checkNotices, 5 * 60 * 1000);
}
